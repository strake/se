...
module Process (CreateProcess (..), proc, readCreateProcess, readCreateProcessLazy, shell) where
...
import System.Process (CreateProcess (..), proc, shell, waitForProcess, withCreateProcess)
import System.Process.Common (ListLikeProcessIO (..), ProcessResult (..))
import System.Process.Text ()
import System.Process.Text.Lazy ()

readCreateProcess :: (ProcessResult a b, ListLikeProcessIO a c) => CreateProcess -> a -> IO b
readCreateProcess = readCreateProcessLazy

readCreateProcessLazy
 :: (ProcessResult a b, ListLikeProcessIO a c) => CreateProcess -> a -> IO b
readCreateProcessLazy maker input = withCreateProcess maker \ inh outh errh pid -> do
...
readInterleaved
 :: (ListLikeProcessIO a c, ProcessResult a b, Foldable f)
...
readInterleaved'
 ...
 => f (a -> b, Handle) -> IO b -> MVar (Either Handle b) -> IO b
readInterleaved' pairs finish res = do
  traverse_ (forkIO . uncurry readHandle) pairs
  takeChunks (length pairs)
    where
      -- Forked thread to read the input and send it to takeChunks via the MVar.
